...
class FCContractOrderView: UIView {
    ...
    var marginModeBtn = fc_buttonInit(imgName: "trade_downtriangle", title: "全仓模式", fontSize: 14, titleColor: COLOR_InputText, bgColor: .clear)
    ...
    var contractModel: FCContractsModel? {
        didSet {
            ...
            /// 永续合约 symbol
            let title = contractModel.symbol?.replacingOccurrences(of: "-", with: "/") ?? "--/--"
            self.symbolBtn.setTitle("\(title)永续", for: .normal)
    ...
    lazy var dropMoreSettingDown: DropDown = {
        ...
        dropMoreSettingDown.dataSource = ["  合约设置  ", "  合约详情  ", "  添加自选  "]
    ...
    lazy var priceTxd: UITextField = {
        ...
        priceTxd.setValue(10, forKey: "paddingLeft")
    ...
    lazy var amountTxd: UITextField = {
        ...
        amountTxd.setValue(10, forKey: "paddingLeft")
    ...
    lazy var priceUnitLab: UILabel = {
        let priceUnitLab = fc_labelInit(text: "USDT", textColor: COLOR_CharTipsColor, textFont: 16, bgColor: COLOR_BGColor)
    ...
    lazy var amountUnitLab: UILabel = {
        let amountUnitLab = fc_labelInit(text: "张", textColor: COLOR_CharTipsColor, textFont: 16, bgColor: COLOR_BGColor)
    ...
    lazy var depthTableView: UITableView = {
        let depthTableView = UITableView.init(frame: .zero, style: .plain)
        depthTableView.isScrollEnabled = false
        depthTableView.separatorInset = .zero
        
        depthTableView.separatorColor = COLOR_HexColorAlpha(0x141416, alpha: 0.7)
    ...
    func loadLeftView () {
        ...
        // 价格
        let priceUnitView = UIView.init(frame: .zero)
        ...
        // 数量
        let amountUnitView = UIView.init(frame: .zero)
        ...
        amountTxd.rx.text.orEmpty.asObservable().subscribe(onNext: { (text) in
            ...
            if FCTradeSettingconfig.sharedInstance.tradeTradingUnit == .TradeTradingUnitType_CONT {
                ...
                /// 张 =？币
                self.convertLab.text = String(format:"≈%.4f \(arrayStrings.first ?? "")", (value * size))
            }else {
        ...
        volumeTitleLab.snp.makeConstraints { (make) in
            make.top.equalTo(slider.snp_bottom).offset(30)
            make.left.equalToSuperview()
        }
        ...
        orderBtn.snp.makeConstraints { (make) in
            ...
            make.top.greaterThanOrEqualTo(estimateLab.snp_bottom).offset(38)
        ...
        orderBtn.rx.tap.subscribe { (event) in
            ...
            let entrustPrice = self.priceTxd.text ?? ""
...
//                //volumeType = "Percentage"
//
//                let amountDouble = Double(self.amountTxd.text ?? "0")!
//                let volDouble = Double(self.volumeLab.text ?? "1")!
//
//                entrustVolume = String(format: "%lf", amountDouble/volDouble)
//                    //String(format: "%.4f", self.slider.value)
//
//                print("entrustVolume: ", entrustVolume,amountDouble,volDouble, amountDouble/volDouble)
//            }
            
            /// 合约下单
            self.placeOrder?(tradingUnit, entrustVolume, entrustPrice, side, action, tradeType, volumeType)
    ...
    func loadDepthDatas(depthModel: FCKLineRestingModel?) {
        ...
        print("depthModel?.bids : ", depthModel?.bids?.count, depthModel?.asks?.count)
